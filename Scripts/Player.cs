...
public class Player : MonoBehaviour
{
    ...
    [HideInInspector] public List<int[]> dominos;

    private List<GameObject> dominosButtons = new List<GameObject>();
    ...
    public void UpdateDominos()
    {
        ...
        int xStart;
        if (count % 2 == 0) { xStart = (step/2 +  ((count / 2) - 1) * step) * (-1); }
        else { xStart = ((count - 1) / 2) * step * (-1); }
